                ...
                <div class="lg:col-span-2 space-y-8">
                    ...
                    <article class="bg-neutral-900/20 border border-neutral-800/80 rounded-2xl overflow-hidden group transition-all duration-300 hover:border-neutral-700">
                        <div class="relative aspect-video w-full bg-neutral-950 overflow-hidden">
                            @if($news[0]->photo)
                                <img src="{{ url('storage/blog/' .$news[0]->id.'/'.$news[0]->photo) }}" alt="{{ $news[0]->title }}" class="w-full h-full object-cover opacity-80 group-hover:scale-105 transition-transform duration-500">
                            @else
                ...
                NOTICIA EXCLUSIVA
                        ...
                        <div class="p-6 sm:p-8">
                            <div class="flex items-center gap-4 text-xs font-mono text-neutral-500 mb-3">
                                <span>{{ $news[0]->created_at->diffForHumans() }}</span>
                            </div>
                            ...
                            <a href="{{ url('sports-news-detail/' . $news[0]->source) }}" class="inline-flex items-center gap-1.5 text-xs font-mono text-lime-400 uppercase tracking-wider font-bold hover:underline">
                                Leer noticia completa <i data-lucide="arrow-up-right" class="w-4 h-4"></i>
                            </a>
                        </div>
                    </article>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        @if(isset($news[1]))
                            <article class="bg-neutral-900/20 border border-neutral-800/40 rounded-xl p-5 hover:border-neutral-700/80 transition-all">
                                ...
                                <a href="{{ url('sports-news-detail/' . $news[1]->source) }}" class="inline-flex items-center gap-1.5 text-xs font-mono text-lime-400 uppercase tracking-wider font-bold hover:underline">
                                    Leer noticia completa <i data-lucide="arrow-up-right" class="w-4 h-4"></i>
                                </a>
                            </article>
                        @endif
                        ...
                        @if(isset($news[2]))
                            <article class="bg-neutral-900/20 border border-neutral-800/40 rounded-xl p-5 hover:border-neutral-700/80 transition-all">
                                ...
                                <a href="{{ url('sports-news-detail/' . $news[2]->source) }}" class="inline-flex items-center gap-1.5 text-xs font-mono text-lime-400 uppercase tracking-wider font-bold hover:underline">
                                    Leer noticia completa <i data-lucide="arrow-up-right" class="w-4 h-4"></i>
                                </a>
                            </article>
                        @endif
