<script type="application/javascript">
    ...
    $("body").on('submit', '#frm-faq', function (event) {
        event.preventDefault()
        if ($('#frm-faq').valid()) {

            var formData = new FormData(document.getElementById("frm-faq"));
                    ...
                    for (var key in errors) {
                        if (errors.hasOwnProperty(key)) {
                            ...
                            if (errors[key][0] === 'validation.unique') {
                                toastr.error('Ha ocurrido un error. la pregunta ya se encuentra registrado');
                                return;
                            }
                        }
                    }
