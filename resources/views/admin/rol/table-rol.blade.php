...
@section('content')
    ...
    <x-admin.table-default
            routeExport=""
            cancelRoute="{{route('rol.index')}}"
            paginate="{{ $roles->appends(request()->input())->links('pagination::bootstrap-4') }}"
            :filter="$filter"
            :showStatus="true"
    >
        <thead>
...
@section('script')
    @include('admin.global_script.function-form-modal', ['route' => route('rol.create')])
    @include('admin.rol.function-rol')
@endsection
